...
class Model(object):
    ...
    def __init__(self,
        ...
        self.mask_index = 0
        self.s_input_step = self.s_max_len
        self.s_hidden = self.s_hidden  # 32
        self.q_input_step = self.q_max_len
        self.q_hidden = self.q_hidden  # 32
        ...
        self.accuracy_train = tf.summary.scalar('accuracy_train', self.accuracy)
        ...
        self.train_summary_ops = tf.summary.merge([self.loss_train, self.accuracy_train])

        self.loss_val = tf.summary.scalar('loss_val', self.loss)
        self.acc_val = tf.summary.scalar('acc_val', self.accuracy)
        self.val_summary_ops = tf.summary.merge([self.loss_val, self.acc_val])
    ...
    def build(self, is_train = True):
        ...
        self.g_units = [256,256,256,256]
        self.f_units = [256,512,159]
        ...
        def g_theta(RN_input, scope='g_theta', reuse = True, phase = True):
            ...
            with tf.variable_scope(scope, reuse = reuse) as scope:
                g_1 = batch_norm_relu(RN_input, self.g_units[0], scope= 'g_1', phase = phase)
                g_2 = batch_norm_relu(g_1, self.g_units[1], scope='g_2', phase=phase)
                g_3 = batch_norm_relu(g_2, self.g_units[2], scope='g_3', phase=phase)
                g_4 = batch_norm_relu(g_3, self.g_units[3], scope='g_4', phase=phase)
            g_output = tf.reshape(g_4, shape= [-1, self.batch_size, self.g_units[3]])
            return g_output
        ...
        def f_phi(g, scope = "f_phi", reuse = True , phase = True):
            ...
            with tf.variable_scope(scope, reuse = reuse) as scope:
                f_1 = batch_norm_relu(f_input, self.f_units[0], scope = "f_1", phase = phase)
                f_2 = batch_norm_relu(f_1, self.f_units[1], scope = "f_2", phase = phase)
                f_3 = batch_norm_relu(f_2, self.f_units[2], scope = "f_3", phase = phase)
            return f_3

        embedded_c = contextLSTM(self.context, self.label, self.context_real_len, reuse = None)
        embedded_q = questionLSTM(self.question, self.question_real_len, reuse = None)
        RN_input = convert_to_RN_input(embedded_c, embedded_q)
        f_input = g_theta(RN_input, reuse = None, phase = self.is_training)
        pred = f_phi(f_input, reuse = None, phase = self.is_training)
